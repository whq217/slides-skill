...
// Count slides — support both .slide (plain) and .reveal .slides > section (Reveal.js)
const isReveal = await page.evaluate(() => typeof Reveal !== 'undefined');
const slideSelector = isReveal ? '.reveal .slides > section' : '.slide';
const slideCount = await page.evaluate((sel) => {
  return document.querySelectorAll(sel).length;
}, slideSelector);

console.log(`  Found ${slideCount} slides (${isReveal ? 'Reveal.js' : 'plain HTML'} mode)`);

if (slideCount === 0) {
  console.error('  ERROR: No slide elements found.');
  console.error('  Expected: .slide elements OR .reveal .slides > section elements.');
  await browser.close();
  server.close();
  process.exit(1);
}
...
for (let i = 0; i < slideCount; i++) {
  // Navigate to slide using the appropriate strategy
  await page.evaluate((index, sel, useReveal) => {
    if (useReveal && typeof Reveal !== 'undefined') {
      // Reveal.js navigation
      Reveal.slide(index, 0, 0);
    } else {
      // Plain HTML slide navigation
      ...
      // If there's a SlidePresentation class instance, use it
